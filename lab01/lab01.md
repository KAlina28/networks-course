...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Accept-Language: ru,ja;q=0.9,en;q=0.8,en-US;q=0.7,ru-RU;q=0.6\r\n
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - да
   - через Line-based text data: text/html (10 lines)
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - да If-Modified-Since: Tue, 25 Feb 2025 06:59:01 GMT\r\n
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3
   - 128.119.245.12[Full request URI: http://gaia.cs.umass.edu/wireshark-labs/HTTP-wireshark-file4.html]
   - 128.119.245.12[Full request URI: http://gaia.cs.umass.edu/pearson.png]
   - 178.79.137.164[Full request URI: http://kurose.cslash.net/8E_cover_small.jpg]
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - исходя из времени запроса - > последовательно
